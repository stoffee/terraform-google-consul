...
resource "google_compute_region_instance_group_manager" "consul_clients" {
  ...
  depends_on = ["google_compute_instance_template.consul_clients_public", "google_compute_instance_template.consul_clients_private"]
}
...
resource "google_compute_instance_template" "consul_clients_public" {
  project = "${var.gcp_project_id}"
...
resource "google_compute_instance_template" "consul_clients_private" {
...
resource "google_compute_firewall" "allow_inbound_http_api" {
  ...
  name    = "${var.cluster_name}-rule-external-api-access-cc"
  network = "${var.network_name}"
...
# Specify which traffic is allowed into the cat image request system
# - This Firewall Rule may be redundant depnding on the settings of your VPC Network, but if your Network is locked down,
#   this Rule will open up the appropriate ports.
# - Note that public access to your Consul Cluster will only be permitted if var.assign_public_ip_addresses is true.
# - This Firewall Rule is only created if at least one source tag or source CIDR block is specified.
resource "google_compute_firewall" "allow_inbound_cat_requests" {
  count = "${length(var.allowed_inbound_cidr_blocks_dns) + length(var.allowed_inbound_tags_dns) > 0 ? 1 : 0}"

  project = "${var.network_project_id != "" ? var.network_project_id : var.gcp_project_id}"

  name    = "${var.cluster_name}-rule-external-cat-access"
  network = "${var.network_name}"
  ...
  source_ranges = "${var.allowed_inbound_cidr_blocks_cat_images}"
  source_tags   = ["${var.allowed_inbound_tags_cat_images}"]
  target_tags   = ["${var.cluster_tag_name}"]
}
...
data "template_file" "compute_instance_template_self_link" {
  ...
  template = "${element(concat(google_compute_instance_template.consul_clients_public.*.self_link, google_compute_instance_template.consul_clients_private.*.self_link), 0)}"
}
