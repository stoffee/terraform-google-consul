...
data "template_file" "startup_script_server" {
  template = "${file("${path.module}/modules/install-consul/onboot/startup-script-server.sh")}"
...
module "consul_clients" {
  ...
  allowed_inbound_tags_cat_images        = ["${var.consul_client_cluster_tag_name}"]
  allowed_inbound_cidr_blocks_cat_images = "${var.consul_client_allowed_inbound_cidr_blocks_cat_images}"
...
data "template_file" "startup_script_client" {
  template = "${file("${path.module}/modules/install-consul/onboot/startup-script-client.sh")}"
